...
兩邊給同樣的任務，由兩個全新的 Claude agent 執行：**「截這 3 個報表頁的手機版」**，對象是一個需要登入的內部管理系統（直接給網址）。

| | Claude in Chrome | 這個 skill |
|---|---|---|
| Token | 48,045 | 48,853 |
| 時間 | 93 秒 | **47 秒** |
| 工具呼叫 | 9 次 | 7 次 |
| 手機尺寸 | 500×667 — Chrome 視窗最窄只能到 500 px | **390×844**（真正的 iPhone 尺寸，3 倍解析度） |
| 產出 | JPEG 存在暫存資料夾，要另外搬 | PNG 直接存到指定位置 |
| 整頁截圖 | 不支援 | 支援 |
| 登入 | 用你 Chrome 裡的登入狀態 | 用之前透過這個 skill 存下的登入狀態 |

**結論**：這種小任務兩邊的 token 差不多，總量裡大部分是 agent 本身的固定開銷。skill 快一倍，而且截到的是真正的手機寬度；Chrome 模擬不了 500 px 以下的寬度，只在 390 px 才出現的跑版問題它抓不到。兩邊各只跑一次、而且有直接給網址；頁面或步驟更多時 skill 應該會拉開差距，但這部分還沒實測。
